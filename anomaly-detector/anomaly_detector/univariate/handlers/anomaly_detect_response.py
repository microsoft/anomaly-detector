...
class LastDetectResponse(object):
    def __init__(self, results, period, spectrum_period, sensitivity, granularity, size, model_id, do_fill_up, boundary_version):
        expected_value, upper_margin, lower_margin, anomaly_neg, anomaly_pos, anomaly, severity, boundary_unit, anomaly_score \
            = get_margins(results, sensitivity, model_id, boundary_version, last=True)

        self.lastExpectedValue = expected_value
        self.lastUpperMargin = upper_margin
        self.lastLowerMargin = lower_margin
        self.isNegativeAnomaly = bool(anomaly_neg)
        self.isPositiveAnomaly = bool(anomaly_pos)
        self.isAnomaly = bool(anomaly)
        self.severity = severity
        self.boundaryUnit = boundary_unit
        self.anomalyScore = anomaly_score

        self.period = period
        self.spectrum_period = spectrum_period
        if period != 0:
            suggested_window = 4 * period + 1
        elif granularity.name in default_gran_window:
            suggested_window = default_gran_window[granularity.name] + 1
        else:
            suggested_window = 0
        self.suggestedWindow = suggested_window
        self.level = math.ceil(size / 1000)
        self.model_id = model_id
        self.do_fill_up = do_fill_up


class EntireDetectResponse(object):
    def __init__(self, results, period, spectrum_period, sensitivity, size, model_id, do_fill_up, boundary_version):
        results = results.sort_index()

        expected_value, upper_margin, lower_margin, anomaly_neg, anomaly_pos, anomaly, severity, boundary_units, anomaly_scores \
            = get_margins(results, sensitivity, model_id, boundary_version, False)
        self.expectedValue = expected_value
        self.upperMargin = upper_margin
        self.lowerMargin = lower_margin
        self.isNegativeAnomaly = anomaly_neg
        self.isPositiveAnomaly = anomaly_pos
        self.isAnomaly = anomaly
        self.expectedValue = expected_value
        self.severity = severity
        self.boundaryUnits = boundary_units
        self.anomalyScore = anomaly_scores
